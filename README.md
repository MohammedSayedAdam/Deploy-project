...
# Server details
IP address: `18.197.74.121`

SSH port: `2200`

Item Catalog URL:  **you can access the app via**    
	[http://18.197.74.121.xip.io](http://18.197.74.121.xip.io) 
...
`ssh grader@18.197.74.121 -p 2200 -i ~/.ssh/grader_rsa`
...
cd project4
sudo git clone https://github.com/MohammedSayedAdam/Item-Category project4
```

## Configure the Catalog app
-  Add a new empty module named `__init__.py`
- To add it run the command `sudo touch __init__.py `
...
## Configure the web application to connect to the PostgreSQL database instead of a SQLite database
 ```
 sudo nano /var/www/item-project/project4/fourth.py
 -Change the engine setting in the file from 'sqlite:///catalog.db' to 'postgresql://catalog:catalog@localhost/catalog', and save
``` 

## Configure the Database to connect to the PostgreSQL database instead of a SQLite database
  ```
 sudo nano /var/www/item-project/project4/database_setup.py
 -Change the engine setting in the file from 'qlite:///catalog.db' to 'postgresql://catalog:catalog@localhost/catalog', and save
 ```
...
```
        ServerName 18.197.74.121
	ServerAdmin albombo2014@gmail.com
	WSGIScriptAlias / /var/www/item-project/app.wsgi
	<Directory /var/www/item-catalog/project4/>
		Order allow,deny
		Allow from all
	</Directory>
...
## Attribution 
- **[Ibrahim Hasan](https://github.com/hima-Megahed)** Thanks for a neat and straightforward README
